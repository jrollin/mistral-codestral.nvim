...
**Quick links:**
- For 30-second verification, see [quick-start.md](quick-start.md)
...
```bash
# Test that API is responding (adjust <plugin-dir> to your plugin installation path)
nvim --headless -u ~/.config/nvim/init.lua -c "luafile <plugin-dir>/tests/api_test.lua" 2>&1
```
...
**Where `<plugin-dir>` is:**
- **lazy.nvim**: `~/.local/share/nvim/lazy/mistral-codestral.nvim` (Linux), `~/.config/nvim/lazy/mistral-codestral.nvim` (macOS), or `$APPDATA\AppData\Local\nvim-data\lazy\mistral-codestral.nvim` (Windows)
- **packer.nvim**: `~/.local/share/nvim/site/pack/packer/start/mistral-codestral.nvim` (Linux/macOS)
...
- [CONFIGURATION.md](CONFIGURATION.md) - Command and configuration reference
