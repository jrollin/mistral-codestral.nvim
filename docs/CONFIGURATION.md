...
require("mistral-codestral").setup({
  ...
  -- Exclusions
  exclusions = {
    filetypes = { "help", "qf", "neo-tree", "neo-tree-popup", "alpha", "dashboard", "nvim-tree", "trouble", "lspinfo", "mason", "lazy", "TelescopePrompt", "TelescopeResults" },
    buffer_patterns = { "^term://", "^%[Command Line%]", "^neo%-tree", "^NvimTree", "^%[Scratch%]" },
    buftypes = { "help", "quickfix", "terminal", "prompt", "nofile" },
  },
  ...
  workspace_root = {
    use_lsp = true,
    find_root = nil,
    paths = { ".git", ".svn", ".hg", "package.json", "Cargo.toml", "pyproject.toml", "go.mod", "requirements.txt" },
  },
})
```
...
Global enable/disable of the plugin.

```lua
enabled = true   -- Default: enabled
enabled = false  -- Disable completions globally
```

Use `:MistralCodestralToggle` to toggle at runtime.
...
```lua
debug = false   -- Default: disabled (no extra output)
debug = true    -- Show debug notifications and logs
```

Check output with `:messages` when enabled.
...
### `virtual_text.filetypes` (table)

Filetype-specific enabling/disabling of virtual text.

```lua
virtual_text = {
  filetypes = {},     -- Default: empty (use default_filetype_enabled)
  -- Or enable for specific filetypes only:
  filetypes = {
    lua = true,
    python = true,
    javascript = false,
...
### `virtual_text.default_filetype_enabled` (boolean)

Whether virtual text is enabled by default for filetypes not explicitly listed.

```lua
virtual_text = {
  default_filetype_enabled = true    -- Default: enabled for all filetypes
  default_filetype_enabled = false   -- Disable by default, only enable in filetypes table
}
```

**Use case**: Set to `false` and add specific filetypes to `filetypes` to enable completion in only certain languages.
...
exclusions = {
  ...
  buffer_patterns = {
    ...
    "^%[Command Line%]",     -- Vim command line
    "^neo%-tree",            -- Neo-tree file browser
    "^NvimTree",             -- NvimTree file browser
